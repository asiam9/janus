...
<feed xmlns="http://www.w3.org/2005/Atom" xmlns:opds="http://opds-spec.org/2010/catalog">
	<title>Janus Series Category Index</title>
	...
	<entry>
		...
		<id>janus:series:section:${section.name?lower_case?xml}</id>
		
		<#if section.contents?size == 1>
		<content type="text">${section.contents?first.name?xml}</content>
		<#elseif section.contents?size &gt; 1>
		<content type="text">${section.contents?size} series from ${section.contents?first.name?xml} to ${section.contents?last.name?xml}</content>
		<#else>
		<content type="text">No series in this section.</content>
		</#if>
		
		<link href="./series_list.xml?starts=${section.id?xml}&amp;mode=${mode}&amp;sort=${sort}" type="application/atom+xml;type=feed;profile=opds-catalog"/>
